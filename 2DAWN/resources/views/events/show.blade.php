...
<section class="bg-white min-h-screen pt-4 pb-16">
  <div class="max-w-6xl mx-auto px-6">
    ...
    <nav class="flex mb-8 text-[13px] font-medium text-gray-400">
        <a href="{{ route('events.index') }}" class="hover:text-tix-orange">Events</a>
        ...
        <div class="flex flex-col gap-4">
            ...
            @else
                <a href="{{ route('events.buy', $event) }}" 
                   class="w-full py-4 px-8 rounded-2xl bg-black text-white font-bold text-lg text-center shadow-lg hover:bg-gray-800 hover:-translate-y-0.5 transition-all duration-300">
      ...
      <div class="lg:col-span-8 flex flex-col gap-10">
        ...
        @if($event->whatsapp_group_url)
            <div>
                <a href="{{ $event->whatsapp_group_url }}" target="_blank" rel="noopener noreferrer"
                   class="inline-flex items-center gap-3 px-6 py-3.5 rounded-2xl bg-[#25D366] text-white font-bold text-sm hover:scale-105 transition-transform shadow-md">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.414 0 0 5.414 0 12.05c0 2.123.551 4.197 1.595 6.02L0 24l6.135-1.61a11.751 11.751 0 005.91 1.611h.005c6.637 0 12.05-5.414 12.05-12.05a11.815 11.815 0 00-3.487-8.522z"/></svg>
        ...
        <div class="mt-8 pt-8 border-t border-gray-100">
             <div class="flex items-center justify-between gap-6">
                 ...
                 @else
                    <button disabled class="px-8 py-3 rounded-xl bg-gray-100 text-gray-400 font-bold cursor-not-allowed">
                        Sold Out
                    </button>
                 @endif
             </div>
...
<section class="bg-gray-50 py-16">
    <div class="max-w-4xl mx-auto px-6">
        ...
        @if (session('status'))
            <div class="mb-8 p-4 rounded-2xl bg-white text-black border border-gray-200 text-sm font-bold shadow-sm">{{ session('status') }}</div>
        @endif
        ...
        <div class="mt-12 bg-white rounded-[32px] p-8 sm:p-10 shadow-sm border border-gray-100">
            ...
            <form method="POST" action="{{ route('events.comments.store', $event) }}" class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div class="sm:col-span-2">
                    <button class="w-full sm:w-auto px-10 py-4 rounded-2xl bg-black text-white font-black hover:scale-105 transition-transform">Post Comment</button>
                </div>
            </form>
        </div>
        ...
        @php
            ...
            $hasSocial = $organizer && ($organizer->instagram_handle || $organizer->whatsapp_number || $organizer->twitter_handle);
        @endphp
        @if($hasSocial)
            <div class="mt-12 bg-white rounded-[32px] p-8 sm:p-10 shadow-sm border border-gray-100">
                <h3 class="text-lg font-black text-gray-900 mb-2">Contact Organizer</h3>
                <p class="text-sm text-gray-600 mb-6">Have questions? Reach out to the organizer</p>
                
                <div class="flex flex-wrap gap-4">
                    @if($organizer->instagram_handle)
                        <a href="https://instagram.com/{{ $organizer->instagram_handle }}" 
                           target="_blank" rel="noopener noreferrer"
                           class="inline-flex items-center gap-3 px-6 py-3 rounded-xl bg-black text-white font-bold hover:scale-105 transition-transform shadow-md">
                    ...
                    @if($organizer->whatsapp_number)
                        <a href="https://wa.me/{{ $organizer->whatsapp_number }}" 
                           target="_blank" rel="noopener noreferrer"
                           class="inline-flex items-center gap-3 px-6 py-3 rounded-xl bg-green-500 text-white font-bold hover:scale-105 transition-transform shadow-md">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    @if($organizer->twitter_handle)
                        <a href="https://twitter.com/{{ $organizer->twitter_handle }}" 
                           target="_blank" rel="noopener noreferrer"
                           class="inline-flex items-center gap-3 px-6 py-3 rounded-xl bg-blue-500 text-white font-bold hover:scale-105 transition-transform shadow-md">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
