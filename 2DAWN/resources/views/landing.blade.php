            ...
            <div class="mb-10">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('events.index') }}"
                       class="text-sm font-medium text-gray-500 hover:text-[#8b5cf6] transition-colors">
                        View all
                    </a>
                </div>
                                ...
                                <div class="relative h-[370px] sm:h-[390px]">
                                    ...
                                    @if($event->capacity !== null && $event->capacity <= 0)
                                        <div class="absolute inset-0 bg-black/60 flex items-center justify-center z-10">
                                            <span class="text-white font-extrabold text-xs uppercase tracking-widest px-2.5 py-1 rounded bg-red-600">Sold Out</span>
                                        </div>
                                    @endif
                ...
                <a href="{{ $event->public_url }}"
                   class="flex items-center rounded-2xl border border-gray-100 bg-white hover:shadow-xl hover:border-gray-200 transition-all duration-300 overflow-hidden group h-44">
                    ...
                    <div class="w-32 sm:w-40 h-full relative bg-gray-50 shrink-0">
                        ...
                        @if($event->capacity !== null && $event->capacity <= 0)
                            <div class="absolute inset-0 bg-black/60 flex items-center justify-center z-10">
                                <span class="text-white font-extrabold text-[10px] uppercase tracking-widest px-2 py-0.5 rounded bg-red-600">Sold Out</span>
                            </div>
                        @else
                    ...
                    <div class="flex-1 p-3 sm:p-4 flex flex-col justify-between min-w-0 h-full">
                        <div>
                            <h3 class="text-sm sm:text-[15px] font-bold text-gray-900 line-clamp-2 transition-colors group-hover:text-[#8b5cf6] leading-snug">
                                {{ $event->title }}
                            </h3>
                            ...
                            @if($event->description)
                                <div class="mt-2 text-[12px] text-gray-400 line-clamp-2 leading-relaxed">
                                    {{ Str::limit(strip_tags($event->description), 100) }}
                                </div>
                            @endif
                        </div>
                        <div>
                            <span class="text-[15px] font-black text-[#8b5cf6]">
                    ...
                    @foreach($otherEvents as $event)
                        ...
                        <a href="{{ $event->public_url }}"
                           class="flex rounded-2xl border border-eventbrite-gray-100 bg-white hover:border-eventbrite-gray-400 hover:shadow-md transition overflow-hidden group">
                            <div class="w-20 sm:w-24 relative bg-eventbrite-gray-50 shrink-0">
                                @if($event->image_url)
                                    <img src="{{ $event->image_url }}" alt="{{ $event->title }}"
                                         class="absolute inset-0 h-full w-full object-cover group-hover:scale-105 transition-transform duration-500 ease-out"/>
                                @else
                                ...
                                @if($event->capacity !== null && $event->capacity <= 0)
                                    <div class="absolute inset-0 bg-black/60 flex items-center justify-center z-10">
                                        <span class="text-white font-extrabold text-[8px] uppercase tracking-widest px-1 py-0.5 rounded bg-red-600">Sold Out</span>
                                    </div>
                                @endif
                            </div>
