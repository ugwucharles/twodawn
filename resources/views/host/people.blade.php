...
<section class="py-8 sm:py-10">
  <div class="max-w-6xl mx-auto px-6">
    ...
    <div class="flex items-start justify-between gap-4 flex-wrap">
      <div>
        <h1 class="text-2xl font-extrabold mt-5">Scanned people — {{ $event->title }}</h1>
        <div class="text-zinc-400 text-sm mt-1">{{ $host->label ? ('Token: '.$host->label) : '' }}</div>
      </div>
      <form method="GET" action="{{ route('host.people.export', $host->token) }}" class="flex flex-wrap sm:flex-nowrap items-center gap-2 w-full sm:w-auto">
        ...
        <button class="inline-flex items-center px-3 py-2 rounded-md bg-white text-black text-sm hover:bg-zinc-100 whitespace-nowrap">Export CSV</button>
      </form>
    </div>
