/**
 * Single resend for Olamide Kuye — corrected email: olamidekuye2021@gmail.com
 */
'use strict';
...
async function main() {
  ...
  // Fetch Olamide's order (ID 27)
  const result = await db.execute({
    sql: `SELECT o.id, o.buyer_name, o.buyer_email, o.buyer_phone, o.amount,
                 o.paystack_reference, o.ticket_code, o.ticket_type, o.quantity,
  ...
  // Override with corrected email
  const correctedEmail = 'olamidekuye2021@gmail.com';
  console.log(`📧 Sending to corrected email: ${correctedEmail}`);
  console.log(`   (Original in DB: ${order.buyer_email})`);
  ...
  const startDate = order.starts_at ? new Date(order.starts_at).toLocaleDateString('en-US',{weekday:'long',year:'numeric',month:'long',day:'numeric'}) : 'TBD';
  const startTime = order.starts_at ? new Date(order.starts_at).toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'}) : 'TBD';
...
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f9fafb; padding: 20px;">
  <div style="background: white; border-radius: 12px; padding: 30px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
    ...
    <div style="background: #f3f4f6; padding: 15px; border-radius: 8px; margin-bottom: 25px;">
      ...
      <p style="color: #1f2937; margin: 0 0 6px; font-size: 14px;"><strong>Name:</strong> ${escapeHtml(order.buyer_name)}</p>
      <p style="color: #1f2937; margin: 0 0 6px; font-size: 14px;"><strong>Email:</strong> ${escapeHtml(correctedEmail)}</p>
      <p style="color: #1f2937; margin: 0; font-size: 14px;"><strong>Phone:</strong> ${escapeHtml(order.buyer_phone || 'Not provided')}</p>
    </div>
    <div style="background: #f3f4f6; padding: 15px; border-radius: 8px; margin-bottom: 25px;">
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 25px;">
      ...
      <div style="background: #f3f4f6; padding: 15px; border-radius: 8px;">
        ...
        <p style="color: #1f2937; margin: 0; font-size: 18px; font-weight: bold;">₦0.00</p>
      </div>
    </div>
  ...
  // Also fix the email in the DB
  await db.execute({
    sql: `UPDATE orders SET buyer_email = ? WHERE id = 27`,
    args: [correctedEmail],
  });
  console.log(`✅ DB updated: buyer_email corrected to ${correctedEmail} for order ID 27`);
