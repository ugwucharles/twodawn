...
// Route::redirect('/dashboard', '/')->name('dashboard'); // disabled to keep current page on refresh
...
Route::middleware(['auth', 'admin', 'throttle:60,1'])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::post('ops/backup', function () {
            try {
                App\Services\BackupService::run(false);
...
Route::prefix('organizer')->name('organizer.')->group(function () {
    // Organizer guest routes (login / register)
    Route::middleware('guest')->group(function () {
            Route::get('/login', [OrganizerLoginController::class , 'create'])->name('login');
            Route::post('/login', [OrganizerLoginController::class , 'store'])->name('login.store');
            Route::get('/register', [OrganizerAuthController::class , 'create'])->name('register');
            Route::post('/register', [OrganizerAuthController::class , 'store'])->name('register.store');
        }
        );

        // Organizer logout (auth required)
        Route::post('/logout', [OrganizerLoginController::class , 'destroy'])->middleware('auth')->name('logout');
...
Route::get('/health', function () {
    return response()->json(['ok' => true, 'time' => now()->toIso8601String()], 200);
});
    ...
    try {
        ...
        $push = function ($loc, $lastmod = null, $changefreq = 'weekly', $priority = '0.7') use (&$urls) {
                ...
                if (view()->exists('about')) {
                    $push(url('/about'), now()->toAtomString(), 'monthly', '0.4');
                }
