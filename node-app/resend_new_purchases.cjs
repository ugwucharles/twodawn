...
async function main() {
  ...
  // Fetch orders after the last known ID (55)
  const result = await db.execute({
    sql: `SELECT o.id, o.buyer_name, o.buyer_email, o.amount, o.paystack_reference, o.created_at,
  ...
  if (orders.length === 0) {
    console.log('No new orders to email.');
    await db.close();
    return;
  }
...
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f9fafb; padding: 20px;">
  <div style="background: white; border-radius: 12px; padding: 30px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
    ...
    <div style="background: #f3f4f6; padding: 15px; border-radius: 8px; margin-bottom: 25px;">
      ...
      <p style="color: #1f2937; margin: 0 0 6px; font-size: 14px;"><strong>Email:</strong> ${escapeHtml(email)}</p>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 25px;">
      ...
      <div style="background: #f3f4f6; padding: 15px; border-radius: 8px;">
        ...
        <p style="color: #1f2937; margin: 0; font-size: 18px; font-weight: bold;">${order.amount > 0 ? `₦${(order.amount/100).toFixed(2)}` : 'FREE'}</p>
      </div>
    </div>
  ...
  <p style="text-align: center; color: #9ca3af; margin-top: 20px; font-size: 12px;">© ${new Date().getFullYear()} 2DAWN. All rights reserved.</p>
    ...
    console.log(`✅ Sent email to ${email} (msgId: ${info.messageId})`);
  }
