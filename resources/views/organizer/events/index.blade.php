            ...
            @forelse($events as $event)
                <div class="p-6 border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    <div class="flex items-center gap-6">
                        <!-- Event Image -->
                        @if($event->image_url)
                            <img src="{{ $event->image_url }}" alt="{{ $event->title }}" class="w-20 h-20 rounded-2xl object-cover">
                        @else
                            <div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-indigo-100 to-indigo-200"></div>
                        @endif
                        ...
                        <div class="flex items-center gap-2">
                            <a href="{{ route('organizer.events.show', $event) }}" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-xl transition-colors">
                                View
                            </a>
                            <a href="{{ route('organizer.events.edit', $event) }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-colors">
                                Edit
                            </a>
                        </div>
                    </div>
