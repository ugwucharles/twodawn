                        ...
                        <tbody class="divide-y divide-gray-50">
                            @foreach($orders as $order)
                                <tr class="hover:bg-gray-50/50 transition-colors">
                                    <td class="px-8 py-6 text-sm font-semibold text-gray-500">{{ $order->paystack_reference }}</td>
                                    <td class="px-8 py-6 text-sm font-black text-gray-900">{{ $order->event->title }}</td>
                                    <td class="px-8 py-6 text-sm font-semibold text-gray-500">
                                        <div class="font-bold text-gray-900">{{ $order->buyer_name }}</div>
                                        <div class="text-xs text-gray-400 font-medium">{{ $order->buyer_email }}</div>
                                    </td>
                                    <td class="px-8 py-6 text-sm font-black text-gray-900">{{ $order->quantity }}</td>
                                    <td class="px-8 py-6 text-sm font-black text-gray-900">₦{{ number_format($order->amount / 100, 2) }}</td>
