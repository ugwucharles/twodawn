...
class CheckoutController extends Controller
{
    public function buy(Event $event)
    {
        ...
        $isSoldOut = !is_null($event->capacity) && $event->capacity <= 0;
        if ($isPast || $isSoldOut) {
    ...
    public function create(Request $request, Event $event)
    {
        ...
        $isSoldOut = !is_null($event->capacity) && $event->capacity <= 0;
        if ($isPast || $isSoldOut) {
            LoggerService::logSecurity('Attempted to buy tickets for past or sold out event', array_merge([
                'event_id' => $event->id,
        ...
        try {
            ...
            // Calculate buyer fees if passed to buyer: 10% per ticket
            // Never add fees for free events (unit price <= 0)
            $feesKobo = 0;
            if ($unitPrice > 0 && $event->pass_fees_to_buyer) {
                $perTicketFeeKobo = (int) round($unitPrice * 0.10 * 100); // 10% kobo
    ...
    public function quote(Event $event, Request $request)
    {
        ...
        try {
            ...
            $feesKobo = 0;
            if ($unitPrice > 0 && $event->pass_fees_to_buyer) {
                $perTicketFeeKobo = (int) round($unitPrice * 0.10 * 100); // 10%
                $feesKobo = max(0, $perTicketFeeKobo * $quantity);
            }
