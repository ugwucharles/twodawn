...
class SecurityHeaders
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        // Restrict camera globally; allow only on admin, organizer, and host scanner routes
        $permissions = 'geolocation=(), microphone=(), camera=()';
        if ($request->is('admin/scanner*') || $request->is('organizer/scanner*') || $request->is('h/*')) {
            $permissions = 'geolocation=(), microphone=(), camera=(self)';
        }
        $response->headers->set('Permissions-Policy', $permissions);
        
        // Base Content Security Policy
        $connectSrc = "connect-src 'self' https://api.paystack.co https://res.cloudinary.com https://cdn.jsdelivr.net https://www.google-analytics.com https://region1.google-analytics.com https://*.google-analytics.com https://www.googletagmanager.com https://plausible.io https://*.sentry.io https://challenges.cloudflare.com";
        $frameSrc = "frame-src https://js.paystack.co https://checkout.paystack.com https://challenges.cloudflare.com";
        $frameAncestors = "frame-ancestors 'none';";

        if (!$isProduction) {
            // Allow Vite HMR and local connections in local development
            $connectSrc .= " ws://localhost:* ws://127.0.0.1:* http://localhost:* http://127.0.0.1:*";
            // Allow same-origin framing for the simulator/emulator in local development
            $frameSrc .= " 'self'";
