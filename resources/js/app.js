...
window.chatWidget = function () {
  ...
  return {
    ...
    async startWithIdentity(){ if (!startUrl) return; if (!this.hasIdentity()) return; this.sending = true; try { const res = await fetch(startUrl, { method: 'POST', headers: { 'Content-Type':'application/json','X-Requested-With': 'XMLHttpRequest', 'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]')?.content || '' }, body: JSON.stringify({ name: (this.name||'').trim(), email: (this.email||'').trim() }) }); const data = await res.json(); if (data.token) { this.token = data.token; this.closed = false; this.msgs = []; this.lastId = 0; this.seenIds = new Set(); localStorage.setItem(tokenKey, data.token); localStorage.setItem('chat_name_v1', (this.name||'').trim()); localStorage.setItem('chat_email_v1', (this.email||'').trim()); this.poll(); this.$nextTick(()=> this.scroll()); } } catch (_) {} finally { this.sending = false; } },
    ...
    async poll() { if (!this.token) return; const run = async () => { try { const res = await fetch(`${msgUrl(this.token)}?after_id=${this.lastId}`, { headers: { 'Accept': 'application/json' } }); const data = await res.json(); if (data && data.messages !== undefined) { if (data.closed) this.closed = true; if (data.messages.length) { data.messages.forEach(m => { if (m && typeof m.id === 'number' && !this.seenIds.has(m.id)) { this.msgs.push(m); this.seenIds.add(m.id); } this.lastId = Math.max(this.lastId, m.id || 0); }); this.$nextTick(() => this.scroll()); } } } catch (_) {} finally { this.timer = setTimeout(run, 4000); } }; run(); },
    ...
    newChat(){ try { localStorage.removeItem(tokenKey); this.token=null; this.closed=false; this.msgs=[]; this.lastId=0; this.seenIds=new Set(); } catch(_){} },
  }
};
